...
compileRecommendCard = function() {
    return '<div id="selector-case" class="card text-white bg-success mb-3"><div class="row g-0"><div class="card-body"><p class="card-text">Recommended peripherals</p></div></div></div>'
};
...
applySearchCriteria = function(searchCriteria) {
    ...
    $.getJSON( config.apiUrl + '/api/product/read.php?category=' + searchCriteria).done(function (data) {
        ...
        if(recommendedCriterias.includes(searchCriteria)) {
            $('#components-array').append(compileRecommendCard());
        }
